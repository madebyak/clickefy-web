    ...
    >
      <div className="max-w-[1240px] mx-auto flex items-center justify-between">
        <div className="flex-1 pointer-events-auto">
          <Link href="/" className="fixed pointer-events-auto z-50">
        ...
        <div className="pointer-events-auto flex items-center gap-2 p-1.5 rounded-full border border-black/5 bg-white/70 backdrop-blur-lg shadow-sm">
          <div className="hidden md:flex items-center gap-1 px-4">
            {nav.links.map((link) => (
              <motion.div key={link.label} whileHover={{ color: "var(--color-ink)" }}>
                <Link
                  href={link.href}
                  className="block px-3 py-2 text-[14px] font-medium text-ink-2 hover:bg-white/5 rounded-full transition-colors"
                >
                  {link.label}
                </Link>
              </motion.div>
